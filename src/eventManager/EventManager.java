	...
	public void actionPerformed(ActionEvent e) {
		...
		switch (e.getActionCommand()) {
         case PanelMain.C_JB_SIMULATE:
        	 ...
        	 try {
        	 Simulation sc = new Simulation(Integer.parseInt(frameMain.getPanelMain().getjTFInitialPoblationFoxes().getText() + "") ,
        			 Integer.parseInt(frameMain.getPanelMain().getjTFInitialPoblationHare().getText() + ""),
        			 Double.parseDouble(frameMain.getPanelMain().getjTFBirthdateFoxes().getText() + ""),
        			 Double.parseDouble(frameMain.getPanelMain().getjTFBirthdateHare().getText() + ""),
        	 ...
        	 for (int i = 1; i <= Integer.parseInt(frameMain.getPanelMain().getjTFYearsSimulate().getText() + ""); i++) {
        		 frameMain.getPanelGraphics().getSimulation().simulateOneYear();
           		 ...
           		 frameMain.getPanelGraphics().setListPointsBirthsFoxes(frameMain.getPanelGraphics().addPoint(frameMain.getPanelGraphics().getListPointsBirthsFoxes(), frameMain.getPanelGraphics().getSimulation().getFoxesThisYear()));
           		 frameMain.getPanelGraphics().setListPointsBirthsHare(frameMain.getPanelGraphics().addPoint(frameMain.getPanelGraphics().getListPointsBirthsHare(), frameMain.getPanelGraphics().getSimulation().getHaresThisYear()));
			}
